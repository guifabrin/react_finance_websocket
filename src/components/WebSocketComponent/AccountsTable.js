...
class Elem extends React.Component {
  ...
  update(accounts) {
    ...
    for (const account of accounts) {
      ...
      for (let month = 0; month < 12; month++) {
        ...
        if (account.is_credit_card) {
          for (const invoice of account.invoices[month]) {
            values.push(
              <div
                key={`subtotal_${account.id}_${invoice.id}_${month}`}
              ...
              >
                ...
                >
                  ...
                  <small className="hide-compact">
                    <small>
                      <NumberFormat value={invoice.total_negative} />
                    </small>
                    <small>
                      <NumberFormat value={invoice.total_positive} />
                    </small>
                  </small>
                </Button>
              </div>
            );
            if (!account.ignore) sumTotals[month] += invoice.total;
          }
        } else {
          values.push(
            <div
              key={`subtotal_${account.id}_${month}`}
              className="actions-buttons"
            >
              <Button
                type="button"
              ...
              <Button
                ...
                variant="link"
                onClick={() =>
                  TransactionsModal.openAccount(account, YearTab.year, month)
                }
              >
                <NumberFormat value={account.values[month]} />
                <small className="hide-compact">
                  <NumberFormat value={account.values_not_paid[month]} />
                </small>
              </Button>
            </div>
          );
          if (!account.ignore) sumTotals[month] += account.values[month];
          if (!account.ignore)
            sumTotalsNotPaid[month] += account.values_not_paid[month];
        }
        totals.push(
          <td
            className={YearTab.isNow(month) ? "table-active" : ""}
            key={`total_${account.id}_${month}`}
          >
            {values}
          </td>
        );
      ...
      tAccounts.push(
        <tr
          ...
          className={account.ignore ? "ignored hide-compact" : ""}
        >
          <th>
            <div style={{ position: "relative", overflow: "hidden" }}>
              ...
              <div className="actions-buttons">
                ...
                <Button type="button" variant="warning" onClick={() => {}}>
                  <FontAwesomeIcon
                    icon={faPen}
                    onClick={() => AccountModal.open(account, accounts)}
                  />
                </Button>
                {account.automated_ref && (
                  <>
                    <img
                      src={imgRef[account.automated_ref]}
                      alt=""
                    ...
                    <Button
                      type="button"
                      variant="primary"
                      onClick={() => this.sync(account)}
                      disabled={account.sync === "init"}
                    >
                      {account.sync === "init" && (
                ...
                {Boolean(account.is_credit_card) && (
                  <>
                    <Button
                      type="button"
                      variant="secondary"
                      onClick={() => InvoicesModal.open(account)}
                    >
                    ...
                    <Button
                      ...
                      variant="secondary"
                      onClick={() => {
                        InvoiceModal.open({ account_id: account.id }, account);
                      }}
                    >
                      <FontAwesomeIcon icon={faPlus} />
                    </Button>
    ...
    this.setState({ table_body: tAccounts });

    const tdListPaid = [<th>{t("accounts.totals_paid")}:</th>];
    const tdListNotPaid = [<th>{t("accounts.totals_not_paid")}:</th>];
    const tdListSumPaid = [<th>{t("accounts.totals")}:</th>];
    for (let month = 0; month < 12; month++) {
      tdListPaid.push(
        <th
          key={`totals_${month}`}
          className={YearTab.isNow(month) ? "table-active" : ""}
        >
          <NumberFormat value={sumTotals[month]} />
        </th>
      );
      tdListNotPaid.push(
        <th
          key={`totals_not_paid_${month}`}
          className={YearTab.isNow(month) ? "table-active" : ""}
        >
          <NumberFormat value={sumTotalsNotPaid[month]} />
        </th>
      );
      tdListSumPaid.push(
        <th
          key={`totals_paid_${month}`}
          className={YearTab.isNow(month) ? "table-active" : ""}
        >
          <NumberFormat value={sumTotals[month] + sumTotalsNotPaid[month]} />
        </th>
      );
    }
    this.setState({
      table_footer: (
        <tfoot key={-1}>
          <tr key={0}>{tdListPaid}</tr>
          <tr key={1}>{tdListNotPaid}</tr>
          <tr key={2}>{tdListSumPaid}</tr>
        </tfoot>
      ),
    });
  }
            ...
            <th>
              ...
              <div className="actions-buttons">
                <Button
                  ...
                  variant="primary"
                  onClick={() => {
                    AccountModal.open({}, this.state.accounts);
                  }}
                >
                  <FontAwesomeIcon icon={faPlus} />
                </Button>
