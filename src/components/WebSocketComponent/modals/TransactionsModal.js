...
class Elem extends React.Component {
  ...
  update(transactions) {
          ...
          <td>
            <Form.Group>
              <Form.Check
                ...
                onChange={() =>
                  this.pay(transaction, !Boolean(transaction.paid))
                }
              />
            </Form.Group>
          </td>
          ...
          <div className="actions-buttons">
            <Button type="button" variant="warning" onClick={() => {}}>
              <FontAwesomeIcon
                icon={faPen}
                onClick={() => {
                  TransactionModal.open(transaction, transaction.sinvoices);
                }}
              />
            </Button>
            <Button
            ...
            <Button
              ...
              variant="primary"
              onClick={() => {
                RepeatTransactionModal.open(transaction);
              }}
            >
    ...
    this.setState({ table_body: <tbody>{trList}</tbody> });
  }
  ...
  open(account, invoice = null, year = null, month = null) {
    ...
    if (invoice) {
      ...
      this.setState({
        title: [
          `${account.id}/${account.description} - ${invoice.id}/${invoice.description} - `,
          <DateFormat value={invoice.debit_date} />,
        ],
        table_header: (
          <tr>
            <th>{t("common.date")}</th>
                ...
                <Button
                  ...
                  onClick={() => {
                    TransactionModal.open(
                      { account_id: account.id, invoice_id: invoice.id },
                      account.sinvoices
                    );
                  }}
                >
      ...
      this.setState({
        title: `${account.id}/${account.description} - ${month + 1}/${year}`,
        table_header: (
          <tr>
            <th>{t("common.date")}</th>
            <th>{t("common.description")}</th>
    ...
    if (invoice) {
      this.setState({
        table_footer: (
          <tfoot key={-1}>
            <tr key={0}>
              <td colSpan="3">{t("accounts.totals_not_paid")}</td>
              <td>
                <NumberFormat value={invoice.total_negative} />
              </td>
            </tr>
            <tr key={1}>
              <td colSpan="3">{t("accounts.totals_paid")}</td>
              <td>
      ...
      <Modal show={this.state.show} onHide={() => this.hide()} size="lg">
        ...
        <Modal.Body>
          <Table striped bordered hover>
            <thead>{this.state.table_header}</thead>
